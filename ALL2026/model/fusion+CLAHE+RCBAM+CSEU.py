...
Strategy: Pre-trained backbones + newly trained fusion head + CLAHE Preprocessing + RCBAM + CSEU.
Outputs :
  OUTPUT_DIR/
    fold_{k}/
    ...
    fusion_mobilenet_resnet_clahe_rcbam_cseu_best_model.pth <- overall best model from all folds
"""
...
OUTPUT_DIR = '/home/eecommu06/Desktop/Bee/ALL/output/fusion_mobilenet_resnet_clahe_rcbam_cseu'
...
class ApplyCLAHE:
    """
    Applies CLAHE to the L-channel of an RGB image to enhance contrast 
    without distorting the color balance of the blood smears.
    """
...
class CSEU(nn.Module):
    """
    Channel and Spatial Excitation Unit (CSEU).
    Combines Channel Squeeze-and-Excitation (cSE) with Spatial Squeeze-and-Excitation (sSE).
    """
    ...
    def forward(self, x):
        # Concurrent excitation: add the feature maps
...
class FeatureFusionModel(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES):
        ...
        if os.path.exists(MOBILENET_WEIGHTS_PATH):
            ...
            print(f"[INFO] Successfully loaded custom MobileNetV2 weights (features only).")
        else:
            print(f"[WARNING] Custom MobileNetV2 weights NOT found at {MOBILENET_WEIGHTS_PATH}!")
            
        self.mobilenet_features = mobilenet.features
        
        # Apply RCBAM and CSEU to MobileNetV2 spatial features (1280 channels)
        ...
        if os.path.exists(RESNET_WEIGHTS_PATH):
            ...
            print(f"[INFO] Successfully loaded custom ResNet50 weights (features only).")
        else:
            print(f"[WARNING] Custom ResNet50 weights NOT found at {RESNET_WEIGHTS_PATH}!")
            
        # Stop before avgpool so we retain spatial dimensions for attention
        self.resnet_features = nn.Sequential(
            resnet.conv1, resnet.bn1, resnet.relu, resnet.maxpool,
            resnet.layer1, resnet.layer2, resnet.layer3, resnet.layer4
        )
        
        # Apply RCBAM and CSEU to ResNet50 spatial features (2048 channels)
    ...
    def forward(self, x):
        # MobileNetV2 Path -> Features -> RCBAM -> CSEU -> Pool -> Flatten
        ...
        # ResNet50 Path -> Features -> RCBAM -> CSEU -> Pool -> Flatten
...
def train_fold(fold, train_idx, val_idx, full_dataset, output_fold_dir):
    ...
    # Base parameters adapt to CLAHE slowly
    base_params = list(model.mobilenet_features.parameters()) + list(model.resnet_features.parameters())
    
    # Head and newly initialized attention modules train at standard LR
...
def main():
    ...
    print(f"  Ablation Step: + CLAHE Preprocessing + RCBAM + CSEU")
    print(f"{'='*64}")
    ...
    if global_best_model_src and os.path.exists(global_best_model_src):
        overall_best_path = os.path.join(OUTPUT_DIR, "fusion_mobilenet_resnet_clahe_rcbam_cseu_best_model.pth")
    ...
    # ── final summary ─────────────────────────────────────────
    print(f"\n{'='*64}")
    print(f"  FINAL 5-FOLD SUMMARY — Fusion (MobileNetV2 + ResNet50 + CLAHE + RCBAM + CSEU)")
    print(f"{'='*64}")
