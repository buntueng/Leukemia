...
Strategy: Pre-trained backbones + newly trained fusion head + CLAHE Preprocessing + RCBAM.
Outputs :
  OUTPUT_DIR/
    fold_{k}/
    ...
    fusion_mobilenet_resnet_clahe_rcbam_best_model.pth <- overall best model from all folds
"""
...
OUTPUT_DIR = '/home/eecommu06/Desktop/Bee/ALL/output/fusion_mobilenet_resnet_clahe_rcbam'
...
class FeatureFusionModel(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES):
        ...
        # Apply RCBAM to MobileNetV2 spatial features (1280 channels)
        ...
        # Stop before avgpool so we retain spatial dimensions for RCBAM
        self.resnet_features = nn.Sequential(
            resnet.conv1, resnet.bn1, resnet.relu, resnet.maxpool,
            resnet.layer1, resnet.layer2, resnet.layer3, resnet.layer4
        )
        
        # Apply RCBAM to ResNet50 spatial features (2048 channels)
    ...
    def forward(self, x):
        # MobileNetV2 Path -> Features -> RCBAM -> Pool -> Flatten
        x1 = self.mobilenet_features(x)
        x1 = self.mobilenet_rcbam(x1)
        x1 = self.mobilenet_pool(x1)
        x1 = torch.flatten(x1, 1)
        
        # ResNet50 Path -> Features -> RCBAM -> Pool -> Flatten
...
def train_fold(fold, train_idx, val_idx, full_dataset, output_fold_dir):
    ...
    # Head and newly initialized RCBAM modules train at standard LR
...
def main():
    ...
    print(f"  Ablation Step: + CLAHE Preprocessing + RCBAM")
    print(f"{'='*64}")
    ...
    if global_best_model_src and os.path.exists(global_best_model_src):
        overall_best_path = os.path.join(OUTPUT_DIR, "fusion_mobilenet_resnet_clahe_rcbam_best_model.pth")
    ...
    # ── final summary ─────────────────────────────────────────
    print(f"\n{'='*64}")
    print(f"  FINAL 5-FOLD SUMMARY — Fusion (MobileNetV2 + ResNet50 + CLAHE + RCBAM)")
    print(f"{'='*64}")
