"""
InceptionV3 — Leukemia Cell Classification
Dataset : CNMC-2019
...
Outputs :
  OUTPUT_DIR/
    ...
    inceptionv3_best_model.pth <- overall best model from all folds
"""
...
OUTPUT_DIR = '/home/eecommu06/Desktop/Bee/ALL/output/inceptionv3'
...
NUM_WORKERS  = 4
SEED         = 42
IMG_SIZE     = 224      # InceptionV3 standard input resolution
...
# ─────────────────────────────────────────────────────────────
# MODEL
# InceptionV3 classifier head is replaced:
#   Original: Linear(2048 -> 1000)
#   Replaced: keep features, new classifier with BN + Dropout
# ─────────────────────────────────────────────────────────────
def build_model(num_classes=NUM_CLASSES):
    # aux_logits=False prevents the model from returning a tuple during training,
    # allowing us to keep the standard training loop unchanged.
    model = inception_v3(weights=Inception_V3_Weights.IMAGENET1K_V1, aux_logits=False)

    # Replace the fully connected layer (in_features = 2048)
...
# ─────────────────────────────────────────────────────────────
# VALIDATE / EVALUATE  (with tqdm batch bar)
# ─────────────────────────────────────────────────────────────
@torch.no_grad()
...
def train_fold(fold, train_idx, val_idx, full_dataset, output_fold_dir):
    ...
    # Two-stage optimizer for InceptionV3:
    #   - Frozen feature layers get a much lower LR (fine-tune gently)
    #   - New classifier head gets the full LR
    ...
    for epoch in epoch_bar:
        ...
        lr_head = optimizer.param_groups[1]["lr"]   # classifier head LR
...
def main():
    ...
    print(f"\n{'='*64}")
    print(f"  InceptionV3 — Leukemia Classification")
    print(f"{'='*64}")
    ...
    print(f"  LR (head)  : {LR}")
    print(f"  LR (feat)  : {LR * 0.1}  (10x lower for frozen feature layers)")
    ...
    if global_best_model_src and os.path.exists(global_best_model_src):
        overall_best_path = os.path.join(OUTPUT_DIR, "inceptionv3_best_model.pth")
    ...
    # ── final summary ─────────────────────────────────────────
    print(f"\n{'='*64}")
    print(f"  FINAL 5-FOLD SUMMARY — InceptionV3")
    print(f"{'='*64}")
