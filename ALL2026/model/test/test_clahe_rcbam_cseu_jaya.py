"""
=============================================================================
Cross-Dataset Blind Test Script — Full Proposed Fusion Model 
(ResNet50 + MobileNetV2 + CLAHE + RCBAM + CSEU + EJAYA)
Training set : CNMC-2019  (5-fold stratified CV)
Test set     : ALL-IDB2   (blind, never seen during training)
Model        : best_model.pth from fold_1
=============================================================================
...
# ─────────────────────────────────────────────
DATASET_DIR  = "/home/eecommu06/Documents/BT/ALLIDB2"   

MODEL_PATH   = "/home/eecommu06/Desktop/Bee/ALL/output/fusion_mobilenet_resnet_full_proposed/fusion_mobilenet_resnet_full_proposed_best_model.pth"
OUTPUT_DIR   = "./test/test_results/fusion_mobilenet_resnet_full_proposed"

BATCH_SIZE   = 32
NUM_WORKERS  = 4
IMG_SIZE     = 257        # Matches the updated training script
...
class ApplyCLAHE(object):
    ...
    to the Luminance (L) channel of an image to preserve color integrity.
    """
...
print(f"[INFO] Class mapping: {test_dataset.class_to_idx}")
...
class EnhancedJAYA(nn.Module):
    """
    Custom JAYA layer applying channel-wise weights r1, r2, and r3.
    """
    def __init__(self, in_channels):
        super(EnhancedJAYA, self).__init__()
        # Initialize as channel-wise tensors [1, C, 1, 1] to match the state_dict
        self.r1 = nn.Parameter(torch.ones(1, in_channels, 1, 1))
        self.r2 = nn.Parameter(torch.ones(1, in_channels, 1, 1))
        self.r3 = nn.Parameter(torch.ones(1, in_channels, 1, 1))

    def forward(self, x):
        # NOTE: If your training code used a different formula for JAYA, change this line!
        return x * self.r1 + x * self.r2 + x * self.r3
...
class FeatureFusionModel(nn.Module):
    def __init__(self, num_classes=2):
        ...
        # --- 1. MobileNetV2 Backbone & Attention ---
        mobilenet = mobilenet_v2(weights=None)
        self.mobilenet_features = mobilenet.features
        self.mobilenet_rcbam = RCBAM(1280)
        self.mobilenet_cseu = CSEU(1280)
        self.mobilenet_ejaya = EnhancedJAYA(1280) # Initialized with 1280 channels
        self.mobilenet_pool = nn.AdaptiveAvgPool2d((1, 1))
        
        # --- 2. ResNet50 Backbone & Attention ---
        resnet = resnet50(weights=None)
        self.resnet_features = nn.Sequential(
            resnet.conv1, resnet.bn1, resnet.relu, resnet.maxpool,
            resnet.layer1, resnet.layer2, resnet.layer3, resnet.layer4
        )
        self.resnet_rcbam = RCBAM(2048)
        self.resnet_cseu = CSEU(2048)
        self.resnet_ejaya = EnhancedJAYA(2048) # Initialized with 2048 channels
...
model = FeatureFusionModel(num_classes=len(CLASS_NAMES))
...
print("\n" + "="*60)
print("  TEST RESULTS — Full Proposed Model (Fusion+CLAHE+RCBAM+CSEU+EJAYA)")
print("="*60)
...
with open(csv_path, "w", newline="") as f:
    ...
    writer.writerow(["Model", "Full Proposed Model (CLAHE+RCBAM+CSEU+EJAYA)"])
...
fig.suptitle("Confusion Matrix — Full Proposed Model",
...
ax.plot(rec_curve, prec_curve, color=PALETTE["secondary"], lw=2.5,
        label=f"Proposed Model  (AP = {ap:.4f})")
...
ax.set_title("Performance Metrics Summary — Full Proposed Model",
