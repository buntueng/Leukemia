"""
VGG19 — Leukemia Cell Classification
Dataset : CNMC-2019
...
OUTPUT_DIR = '/home/eecommu06/Desktop/Bee/ALL/output/vgg19'
...
NUM_WORKERS  = 4
SEED         = 42
IMG_SIZE     = 224       # VGG19 standard input resolution
...
# ─────────────────────────────────────────────────────────────
# TRANSFORMS
# NOTE: VGG19 uses 224×224 (vs 300×300 for EfficientNet-B3)
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# MODEL
# VGG19 classifier head is replaced:
#   Original: AdaptiveAvgPool2d → FC(25088→4096) → ReLU → Dropout
#             → FC(4096→4096) → ReLU → Dropout → FC(4096→1000)
#   Replaced: keep features + avgpool, new classifier with BN + Dropout
# ─────────────────────────────────────────────────────────────
def build_model(num_classes=NUM_CLASSES):
    model = vgg19(weights=VGG19_Weights.IMAGENET1K_V1)

    # Replace the classifier (in_features = 25088 after adaptive pool)
...
def main():
    ...
    print(f"\n{'='*64}")
    print(f"  VGG19 — Leukemia Classification")
    print(f"{'='*64}")
    ...
    # ── final summary ─────────────────────────────────────────
    print(f"\n{'='*64}")
    print(f"  FINAL 5-FOLD SUMMARY — VGG19")
    print(f"{'='*64}")
