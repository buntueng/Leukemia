...
NUM_EPOCHS   = 50
BATCH_SIZE   = 32
LR           = 1e-4        # Standard LR for the head
WEIGHT_DECAY = 1e-4
PATIENCE     = 10        
NUM_WORKERS  = 32         
SEED         = 42
...
train_transform = transforms.Compose([
    ...
    ApplyCLAHE(clip_limit=2.0, tile_grid_size=(8, 8)),  # Apply CLAHE
...
class FeatureFusionModel(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES):
        ...
        # NOTE: We are intentionally NOT freezing the backbones here.
        # They need to fine-tune to adapt to the new CLAHE contrast domain.
...
def train_fold(fold, train_idx, val_idx, full_dataset, output_fold_dir):
    ...
    # Gather parameters for differential learning rates
    base_params = list(model.mobilenet_features.parameters()) + list(model.resnet_features.parameters())
    head_params = model.classifier.parameters()
    
    # Differential learning rate: Backbones train slow, head trains normal
...
def main():
    ...
    print(f"  Ablation Step: + CLAHE Preprocessing")
    print(f"{'='*64}")
    ...
    # ── final summary ─────────────────────────────────────────
    print(f"\n{'='*64}")
    print(f"  FINAL 5-FOLD SUMMARY — Fusion (MobileNetV2 + ResNet50 + CLAHE)")
    print(f"{'='*64}")
