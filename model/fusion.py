...
Strategy: Frozen pre-trained backbones + newly trained fusion head.
Outputs :
  OUTPUT_DIR/
    fold_{k}/
    ...
    fusion_mobilenet_resnet_best_model.pth <- overall best model from all folds
"""
...
OUTPUT_DIR = '/home/eecommu06/Desktop/Bee/ALL/output/fusion_mobilenet_resnet'
...
NUM_EPOCHS   = 50
BATCH_SIZE   = 32
LR           = 1e-4        # Standard LR for training a new classifier head
WEIGHT_DECAY = 1e-4
PATIENCE     = 10        
NUM_WORKERS  = 32         
SEED         = 42
...
# ─────────────────────────────────────────────────────────────
# MODEL FUSION
# Combines frozen feature extraction from MobileNetV2 and ResNet50
# ─────────────────────────────────────────────────────────────
class FeatureFusionModel(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES):
        ...
        # Load your custom weights (Filtering out the old classifier to prevent size mismatch)
        ...
        # FREEZE ResNet50 Features
        for param in self.resnet_features.parameters():
            param.requires_grad = False
            
            
        # --- 3. Unified Classifier Head ---
        # 1280 (MobileNet) + 2048 (ResNet) = 3328 total features
...
def main():
    ...
    if global_best_model_src and os.path.exists(global_best_model_src):
        overall_best_path = os.path.join(OUTPUT_DIR, "fusion_mobilenet_resnet_best_model.pth")
    ...
    # ── final summary ─────────────────────────────────────────
    print(f"\n{'='*64}")
    print(f"  FINAL 5-FOLD SUMMARY — Fusion (MobileNetV2 + ResNet50)")
    print(f"{'='*64}")
