"""
MobileNetV2 — Leukemia Cell Classification
Dataset : CNMC-2019
...
Outputs :
  OUTPUT_DIR/
    ...
    mobilenetv2_best_model.pth <- overall best model from all folds
"""
...
OUTPUT_DIR = '/home/eecommu06/Desktop/Bee/ALL/output/mobilenetv2'
...
NUM_WORKERS  = 4
SEED         = 42
IMG_SIZE     = 224       # MobileNetV2 standard input resolution
...
# ─────────────────────────────────────────────────────────────
# MODEL
# MobileNetV2 classifier head is replaced:
...
def build_model(num_classes=NUM_CLASSES):
    ...
    # Replace the classifier (in_features = 1280 for MobileNetV2)
...
def main():
    ...
    print(f"\n{'='*64}")
    print(f"  MobileNetV2 — Leukemia Classification")
    print(f"{'='*64}")
    ...
    if global_best_model_src and os.path.exists(global_best_model_src):
        overall_best_path = os.path.join(OUTPUT_DIR, "mobilenetv2_best_model.pth")
    ...
    # ── final summary ─────────────────────────────────────────
    print(f"\n{'='*64}")
    print(f"  FINAL 5-FOLD SUMMARY — MobileNetV2")
    print(f"{'='*64}")
