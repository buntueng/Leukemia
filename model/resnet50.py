"""
ResNet50 — Leukemia Cell Classification
Dataset : CNMC-2019
...
Outputs :
  OUTPUT_DIR/
    ...
    resnet50_best_model.pth   <- overall best model from all folds
"""
...
OUTPUT_DIR = '/home/eecommu06/Desktop/Bee/ALL/output/resnet50'
...
# ─────────────────────────────────────────────────────────────
# MODEL
# ResNet50 classifier head is replaced:
...
def train_fold(fold, train_idx, val_idx, full_dataset, output_fold_dir):
    ...
    # Two-stage optimizer for ResNet:
...
def main():
    ...
    print(f"\n{'='*64}")
    print(f"  ResNet50 — Leukemia Classification")
    print(f"{'='*64}")
    ...
    if global_best_model_src and os.path.exists(global_best_model_src):
        overall_best_path = os.path.join(OUTPUT_DIR, "resnet50_best_model.pth")
    ...
    # ── final summary ─────────────────────────────────────────
    print(f"\n{'='*64}")
    print(f"  FINAL 5-FOLD SUMMARY — ResNet50")
    print(f"{'='*64}")
