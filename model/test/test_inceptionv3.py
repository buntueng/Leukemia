...
MODEL_PATH   = "/home/eecommu06/Desktop/Bee/ALL/output/inceptionv3/inceptionv3_best_model.pth"
OUTPUT_DIR   = "./test/test_results/inceptionv32"
BATCH_SIZE   = 32
NUM_WORKERS  = 4
IMG_SIZE     = 257       # InceptionV3 native resolution
...
def build_model(num_classes: int = 2) -> nn.Module:
    # Set aux_logits=False for inference build to match standard fc modification
    model = models.inception_v3(weights=None, aux_logits=False)
    # InceptionV3 uses model.fc instead of model.classifier
...
if isinstance(checkpoint, dict):
    ...
    # strict=False safely ignores auxiliary classifier weights if they exist in the checkpoint
...
print("\n" + "="*60)
print("  TEST RESULTS — InceptionV3 on ALL-IDB2")
print("="*60)
...
# ─────────────────────────────────────────────
csv_path = os.path.join(OUTPUT_DIR, "inceptionv3_metrics_summary.csv")
with open(csv_path, "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Metric", "Value"])
    writer.writerow(["Model", "InceptionV3"])
...
fig.suptitle("Confusion Matrix — InceptionV3 on ALL-IDB2",
...
ax.plot(fpr, tpr, color=PALETTE["primary"], lw=2.5,
        label=f"InceptionV3  (AUC = {roc_auc:.4f})")
...
ax.set_title("Performance Metrics Summary — InceptionV3 on ALL-IDB2",
