"""
=============================================================================
Cross-Dataset Blind Test Script — MobileNetV2 (Custom Head)
...
BATCH_SIZE   = 32
NUM_WORKERS  = 4
IMG_SIZE     = 257        # MobileNetV2 standard resolution
...
def build_model(num_classes: int = 2) -> nn.Module:
    ...
    # Rebuilding the custom classifier based on your checkpoint's state_dict
    model.classifier = nn.Sequential(
        nn.Dropout(p=0.2),                  # 0: standard dropout
        nn.Linear(in_features, 512),        # 1: 1280 -> 512
        nn.BatchNorm1d(512),                # 2: batch norm
        nn.ReLU(),                          # 3: activation
        nn.Dropout(p=0.2),                  # 4: secondary dropout
        nn.Linear(512, num_classes)         # 5: final classification layer
    )
    return model
...
print("\n" + "="*60)
print("  TEST RESULTS — MobileNetV2 on ALL-IDB2")
print("="*60)
...
# ─────────────────────────────────────────────
csv_path = os.path.join(OUTPUT_DIR, "mobilenetv2_metrics_summary.csv")
...
fig.suptitle("Confusion Matrix — MobileNetV2 on ALL-IDB2",
...
ax.set_title("Performance Metrics Summary — MobileNetV2 on ALL-IDB2",
