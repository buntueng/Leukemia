"""
=============================================================================
Cross-Dataset Blind Test Script — VGG19 (Custom Head)
...
MODEL_PATH   = "/home/eecommu06/Desktop/Bee/ALL/output/vgg19/fold_1/best_model.pth"
OUTPUT_DIR   = "./test/test_results/vgg19"
BATCH_SIZE   = 32
NUM_WORKERS  = 4
IMG_SIZE     = 257        # VGG19 standard resolution
...
def build_model(num_classes: int = 2) -> nn.Module:
    ...
    # Rebuilding the custom classifier based on your checkpoint's exact structure.
    # If you get a size mismatch on layer 4, change '1024' to the value in the error log!
    model.classifier = nn.Sequential(
        nn.Linear(25088, 4096),       # 0: VGG default first layer
        nn.BatchNorm1d(4096),         # 1: Batch Norm
        nn.ReLU(True),                # 2: Activation
        nn.Dropout(p=0.5),            # 3: Dropout
        nn.Linear(4096, 1024),        # 4: Hidden Layer (guess based on previous models)
        nn.BatchNorm1d(1024),         # 5: Batch Norm
        nn.ReLU(True),                # 6: Activation
        nn.Dropout(p=0.5),            # 7: Dropout
        nn.Linear(1024, num_classes)  # 8: Final output layer
    )
    return model
...
print("\n" + "="*60)
print("  TEST RESULTS — VGG19 on ALL-IDB2")
print("="*60)
...
# ─────────────────────────────────────────────
csv_path = os.path.join(OUTPUT_DIR, "vgg19_metrics_summary.csv")
with open(csv_path, "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Metric", "Value"])
    writer.writerow(["Model", "VGG19"])
...
fig.suptitle("Confusion Matrix — VGG19 on ALL-IDB2",
...
ax.set_title("Performance Metrics Summary — VGG19 on ALL-IDB2",
