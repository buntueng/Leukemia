"""
=============================================================================
Cross-Dataset Blind Test Script — Xception (Custom Head)
...
# timm is required because Xception is not built into torchvision natively
try:
    import timm
except ImportError:
    raise ImportError("Please install timm to use Xception: pip install timm")
...
BATCH_SIZE   = 32
NUM_WORKERS  = 4
IMG_SIZE     = 257        # Xception standard native resolution
...
def build_model(num_classes: int = 2) -> nn.Module:
    ...
    in_features = model.get_classifier().in_features  # Typically 2048 for Xception
    
    # Rebuilding the custom classifier based on previous script structures
    # If you get a size mismatch on layer 0, change '1024' to match your training setup!
    model.fc = nn.Sequential(
        nn.Linear(in_features, 1024),       # 0: 2048 -> 1024
        nn.BatchNorm1d(1024),               # 1: batch norm for 1024
        nn.ReLU(),                          # 2: activation
        nn.Dropout(p=0.2),                  # 3: dropout
        nn.Linear(1024, num_classes)        # 4: 1024 -> 2 (final output)
    )
    return model
...
print("\n" + "="*60)
print("  TEST RESULTS — Xception on ALL-IDB2")
print("="*60)
...
# ─────────────────────────────────────────────
csv_path = os.path.join(OUTPUT_DIR, "xception_metrics_summary.csv")
with open(csv_path, "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Metric", "Value"])
    writer.writerow(["Model", "Xception"])
...
fig.suptitle("Confusion Matrix — Xception on ALL-IDB2",
...
ax.set_title("Performance Metrics Summary — Xception on ALL-IDB2",
