"""
Xception — Leukemia Cell Classification
Dataset : CNMC-2019
...
Outputs :
  OUTPUT_DIR/
    ...
    xception_best_model.pth   <- overall best model from all folds
"""
...
# timm is required for Xception (pip install timm)
import timm 
...
OUTPUT_DIR = '/home/eecommu06/Desktop/Bee/ALL/output/xception'
...
# ─────────────────────────────────────────────────────────────
# MODEL
# Xception classifier head is replaced:
...
def train_fold(fold, train_idx, val_idx, full_dataset, output_fold_dir):
    ...
    # Two-stage optimizer for Xception:
...
def main():
    ...
    print(f"\n{'='*64}")
    print(f"  Xception — Leukemia Classification")
    print(f"{'='*64}")
    ...
    if global_best_model_src and os.path.exists(global_best_model_src):
        overall_best_path = os.path.join(OUTPUT_DIR, "xception_best_model.pth")
    ...
    # ── final summary ─────────────────────────────────────────
    print(f"\n{'='*64}")
    print(f"  FINAL 5-FOLD SUMMARY — Xception")
    print(f"{'='*64}")
